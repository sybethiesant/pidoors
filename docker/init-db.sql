...
CREATE TABLE IF NOT EXISTS `doors` (
  ...
  `unlock_duration` int(11) DEFAULT 5,
  `status` enum('online','offline','unknown') DEFAULT 'unknown',
  `last_seen` datetime DEFAULT NULL,
  `locked` tinyint(1) DEFAULT 1,
  `held_open` tinyint(1) NOT NULL DEFAULT 0,
  `hold_requested` tinyint(1) NOT NULL DEFAULT 0,
  `lockdown_mode` tinyint(1) DEFAULT 0,
  `reader_type` enum('wiegand','osdp','nfc_pn532','nfc_mfrc522') DEFAULT 'wiegand',
  `controller_version` varchar(20) DEFAULT NULL,
  ...
  `poll_interval` int(11) NOT NULL DEFAULT 3,
  PRIMARY KEY (`id`),
